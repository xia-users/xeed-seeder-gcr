...
init: ## Activation of API, creation of service account with publisher role
	...
	gcloud iam service-accounts create gcr-xia-seeder-02 \
		--display-name "Cloud Run Xeed Http Endpoint"; \
	gcloud projects add-iam-policy-binding $${PROJECT_ID} \
		--member=serviceAccount:gcr-xia-seeder-02@$${PROJECT_ID}.iam.gserviceaccount.com \
...
build: ## Build and upload Cloud Run Image
	@PROJECT_ID=$(shell gcloud config list --format 'value(core.project)'); \
	gcloud builds submit --tag gcr.io/$${PROJECT_ID}/xia-seeder-02;
...
deploy: ## Deploy Cloud Run Image by using the last built image
	...
	gcloud pubsub topics create test-002; \
	gcloud run deploy xia-seeder-02 \
		--image gcr.io/$${PROJECT_ID}/xia-seeder-02 \
		--service-account gcr-xia-seeder-02@$${PROJECT_ID}.iam.gserviceaccount.com \
		--region $${CLOUD_RUN_REGION} \
		--platform managed \
		--allow-unauthenticated \
		--update-env-vars XEED_USER=$${XEED_USER},XEED_PASSWORD=$${XEED_PASSWORD},XEED_DEST=xia-dispatcher-zmfr66omva-ew.a.run.app,XEED_TOPIC=test-002;
...
update: ## Update User/Password
	...
	read -e -p "Enter Desired Username: " -i "user" XEED_USER; \
	read -e -p "Enter Desired Password: " -i "La_vie_est_belle" XEED_PASSWORD; \
	gcloud run services update xia-seeder-02 \
		--region $${CLOUD_RUN_REGION} \
		--platform managed \
